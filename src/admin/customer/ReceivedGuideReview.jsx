...
function ReceivedGuideReview({memberId}) {
    ...
    const getApi = async(memberId, state, page) => {
        console.log("받은 가이드 리뷰를 가져옵니다...")
        console.log(`http://localhost:8099/admin/member/review/guide/receiver?memberId=${memberId}&state=${state}&page=${page}`)
        const response = await axios.get(
            `${process.env.REACT_APP_API_URL}/admin/member/review/guide/receiver?memberId=${memberId}&state=${state}&page=${page}`,
            {
                headers: {
    ...
    return (
        <Grid container item xs={12}>
            <Grid item xs={12} sx={{display:"flex", justifyContent:"flex-start", alignItems:"center", mb:"1rem", mt:"3rem"}}>
                <Typography sx={{fontSize:"2rem", fontWeight:"700", display:"inline"}}>받은 가이드 리뷰</Typography>
...
export default ReceivedGuideReview;