...
function WriteTourReview({memberId}) {
    ...
    const getApi = async(memberId, page, sort) => {
        ...
        console.log(`http://localhost:8099/admin/member/review/member?memberId=${memberId}&page=${page}&state=${sort}`)
        const response = await axios.get(
            `${process.env.REACT_APP_API_URL}/admin/member/review/member?memberId=${memberId}&page=${page}&state=${sort}`,
            {
                headers: {
    ...
    useEffect(() => {
        setPage(0);
        if(accessToken){
            getApi(memberId, 0, parseInt(sort));
        }
    }, [sort])
...
export default WriteTourReview;