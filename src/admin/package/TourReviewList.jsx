...
function TourReviewList({tourId}) {
    ...
    const getApi = async(tourId, state, page) => {
        ...
        console.log(`http://localhost:8099/admin/review/tour/list?tourId=${tourId}&state=${state}&page=${page}`)
        const response = await axios.get(
            `${process.env.REACT_APP_API_URL}/admin/review/tour/list?tourId=${tourId}&state=${state}&page=${page}`,
            {
                headers: {
            ...
            if(res && res.data){
                console.log(res);
                setResult(res.data.contentReviews);
    ...
    const deleteApi = async(reviewId) => {
        ...
        const response = await axios.post(
            ...
            console.log(res);
            getApi(tourId, parseInt(sort), page); // 재로드
        }).catch((err) => console.log(err))
                            ...
                            {result && result.map((item, idx) => {
                                return(
                                    <TableRow key={idx}>
                                        ...
                                        <TableCell>{item.memeberName}</TableCell>
